  ...
  <section class="h-screen flex-col justify-center">
    ...
    <div class="w-[23%] mx-auto mt-14 text-sm">
      <div class="flex">
        ...
        <div class="w-1/2">
          ...
          <input
            type="email"
            v-model="email"
            class="mt-2 mb-4 box-border ml-[5%] rounded-md w-[95%] border-0 py-2 text-gray-900 shadow-sm ring-1 p-2 ring-inset ring-gray-300 focus:ring-orange-400 focus:ring-2"
          />
        </div>
      </div>
      ...
      v-model="password"
        type="password"
        accept=".jpg,.png,.jpeg"
        class="mt-2 mb-4 w-full rounded-md border-0 py-2 text-gray-900 shadow-sm ring-1 p-2 ring-inset ring-gray-300 focus:ring-orange-400 focus:ring-2"
      />
      ...
      <button @click="newUser" class="w-full bg-orange-600 text-sm font-semibold text-white mt-8 py-2 rounded tracking-tight">Create account</button>
        <p class="font-medium tracking-tight text-sm mt-4 text-gray-900">Already a member? Log in <router-link to="/login" class="text-orange-500"> here</router-link> </p>
    </div>
  </section>
</template>
<style>
...
const handleFileUpload = async (e) => {
  ...
  console.log(image);
  const { data, error } = await supabase.storage.from("profilePictures").upload(`public/${email.value}`, image);
  if (error) {
    console.log(error.message);
  } else {
